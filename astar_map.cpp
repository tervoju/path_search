...
private:
    ...
    vector<vector<double>> GetMap(void)
    {
    ...
    vector<vector<int>> MaptoGrid(void)
    {
        ...
        */
        for (int idx = 0; idx < this->mapHeight; idx++)
        {
            for (int idy = 0; idy < this->mapWidth; idy++)
            {
                if (this->map[idx][idy] < 0)
                {
                    grid[idx][idy] = 0;
                }
                else
    ...
    vector<vector<int>> GenerateHeuristic(void)
    {
        int goal[2] = {60, 50};
        ...
        for (int idx = 0; idx < this->mapHeight; idx++)
        {
            for (int idy = 0; idy < this->mapWidth; idy++)
            {
                heuristic[idx][idy] = abs(goal[0]- idx) +  abs(goal[1] - idy);
            }
        }
        return heuristic;
    }
